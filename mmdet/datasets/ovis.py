...
class OVISDataset(CustomDataset):
    ...
    def prepare_train_img(self, idx):
        ...
        ref_imgname = osp.join(osp.join(self.img_prefix, vid_info['filenames'][ref_frame_id]))
        ref_img = mmcv.imread(ref_imgname)
    ...
    def prepare_test_img(self, idx):
        ...
        neobhour = 5
        #prepare ref imgs
        start = max(0,frame_id - neobhour)
        end = min(frame_id+neobhour+1,len(vid_info['filenames']))
        ref_frames_ids = []
        for ref_id in range(start,end):
            if(ref_id==frame_id):
                continue 
            ref_frames_ids.append(ref_id)

        img = mmcv.imread(osp.join(self.img_prefix, vid_info['filenames'][frame_id]))
        ref_imgs = []
        for ref_frames_id in ref_frames_ids:
            ref_filename = osp.join(self.img_prefix,vid_info['filenames'][ref_frames_id])
            ref_imgs.append(mmcv.imread(ref_filename))
        ...
        for scale in self.img_scales:
            ...
            for ref_img,ref_frame_id in zip(ref_imgs,ref_frames_ids):
                _img_ref,_img_meta_ref,_ = prepare_single(
                    ref_img,ref_frame_id,scale,False,proposal,
                )
                _img_refs.append(_img_ref)
                _img_meta_refs.append(_img_meta_ref)
            
            imgs_ref.append(_img_refs)
            imgs_metas_ref.append(_img_meta_refs)
            imgs.append(_img)
            ...
            if self.flip_ratio > 0:
                ...
                img_metas.append(DC(_img_meta, cpu_only=True))
                proposals.append(_proposal)
        data = dict(img=imgs, img_meta=img_metas,ref_imgs=imgs_ref, ref_img_metas=imgs_metas_ref)
        return data
