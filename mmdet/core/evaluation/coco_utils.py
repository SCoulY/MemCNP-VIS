...
def ovis_eval_vis(result_file, result_types, ovis_dataset, max_dets=(100, 300, 1000)):
    ...
    if isinstance(ovis_dataset, str):
        ovis = ovis_dataset
    elif hasattr(ovis_dataset, 'ovis'):
        ovis = ovis_dataset.ovis
    else:
        ovis = ovis_dataset.ytvos #for Yutube-VIS compatibility
    if mmcv.is_str(ovis):
        ovis = OVIS(ovis)
    assert isinstance(ovis, OVIS) or isinstance(ovis, YTVOS)
...
def results2json_videoseg(dataset, results, out_file, dump_bbox=False):
    json_results = []
    vid_objs = {}
    for idx in range(len(dataset)):
