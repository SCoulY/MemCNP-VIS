...
class FeatureAlign(nn.Module):
    ...
    def forward(self, x, shape):
        if shape.size(2)<self.conv_adaption.kernel_size[0] or shape.size(3)<self.conv_adaption.kernel_size[1]:
...
class SipMaskHead(nn.Module):
    ...
    def loss(self,
        ...
        ###centerness visualisation###
        vis_centerness = [F.interpolate(centerness, centernesses[0].shape[2:], mode='bilinear', align_corners=False)
             for centerness in centernesses]
        vis_centerness = torch.stack(vis_centerness, dim=0).mean(0)[0].squeeze()
        ...
        for i in range(num_imgs):
            ...
            pos_inds_list.append(pos_inds.detach().cpu())
        ...
        return dict(
            ...
            match_acc=match_acc), dict(#tensorboard log objects
            pos_inds_list=pos_inds_list,
            img_metas=img_metas,
            mask_shape_list=mask_shape_list,
            pos_decoded_bbox_preds=pos_decoded_bbox_preds.detach().cpu(),
            pos_decoded_target_preds=pos_decoded_target_preds.detach().cpu(),
            cent_pred=vis_centerness.sigmoid().detach().cpu(),
            cent_target=vis_centerness.sigmoid().detach().cpu())
    ...
    def get_bboxes_single(self,
        ...
        det_bboxes, det_labels, det_cofs = self.fast_nms(mlvl_bboxes, mlvl_scores.transpose(1, 0).contiguous(),
