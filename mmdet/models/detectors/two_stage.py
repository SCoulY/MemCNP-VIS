...
class TwoStageDetector(BaseDetector, RPNTestMixin,
    ...
    def simple_test(self, img, img_meta,ref_imgs,ref_img_metas, proposals=None, rescale=False):
        ...
        if is_first:
            ...
            self.extracted_feats = [self.extract_feat(img)] + [self.extract_feat(ref_img) for ref_img in ref_imgs[0]] #memory queue
        else:
            len_ref = len(ref_imgs[0])
            if len_ref > self.len_ref:
                self.cur_idx += 1
                self.extracted_feats.append(self.extract_feat(ref_imgs[0][-1])) #ref_imgs[scale_idx][frame_idx], use last frame_idx in the sampled ref_clip 
        ...
        bbox_results = bbox2result_with_id(det_bboxes, det_labels, det_obj_ids,
                                   self.bbox_head.num_classes) # bbox of shape (n,5) ie., (n, 4+class_prob)
