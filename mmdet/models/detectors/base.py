...
class BaseDetector(nn.Module):
    ...
    def show_result(self,
        ...
        if isinstance(bbox_result, dict) and len(bbox_result.keys()) == 0:
            if save_vis:
              save_path = os.path.join(save_path, str(img_metas[0]['video_id']), str(img_metas[0]['frame_id'])+'.png')
              h, w, _ = img_metas[0]['img_shape']
              mmcv.imwrite(imgs[0][:h, :w], save_path)
            return
        ...
        for img, img_meta in zip(imgs, img_metas):
            ...
            if segm_result is not None:
                ...
                inds = np.where(bboxes[:, 4] > score_thr)[0]
                for i in inds:
                    ...
                    if not is_video:
                      ...
                      color_id = obj_ids[i]
                    # try:
                    #   img_show[mask] =  img_show[mask]*0.2 +self.color_mask[color_id,:]*0.8
            ...
            if save_vis:
              show = False 
              save_path = os.path.join(save_path, str(img_meta['video_id']), str(img_meta['frame_id'])+'.png')
            ...
            mmcv.imshow_det_bboxes_w_uncert(
                img_show,
                bboxes[inds],
                labels[inds],
