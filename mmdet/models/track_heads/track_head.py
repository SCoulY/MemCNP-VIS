...
class TrackHead(nn.Module):
    ...
    def loss(self,
        ...
        ids = torch.split(ids, x_n, dim=0) #split ids into m parts, each part stands for a scalar instance id label
        loss_match = 0.
        match_acc = 0.
        n_total = 0
        batch_size = len(ids)
        for score, cur_ids, cur_weights in zip(match_score, ids, id_weights):
            ...
            loss_match += weighted_cross_entropy(
                score, cur_ids, cur_weights, reduce=reduce) # score is of shape N (N instances from query), cur_ids is a scalar ranging from 0 to N-1
