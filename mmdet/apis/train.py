...
def batch_processor(model, data, train_mode=False, log_img=False):
    losses, tf_logs = model(**data)
    ...
    if not log_img:
        ...
        outputs = dict(
            loss=loss, log_vars=log_vars, num_samples=len(data['img'].data), tf_logs=tf_logs)
...
def _dist_train(model, dataset, cfg, validate=False, logger=None):
    ...
    runner = Runner(model, batch_processor, cfg.optimizer, cfg.work_dir,
                    cfg.log_level, logger=logger, img_norm_cfg=cfg.img_norm_cfg)
    # register hooks
