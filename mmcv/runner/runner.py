...
from .hooks import (Hook, LrUpdaterHook, CheckpointHook, IterTimerHook,
                    OptimizerHook, lr_updater, MeMCNPHook)
...
class Runner(object):
    ...
    def __init__(self,
        ...
        if mmcv.is_str(work_dir):
            print('runner work dir :{}'.format(work_dir))
    ...
    def train(self, data_loader, **kwargs):
        ...
        log_img = any(isinstance(x, mmcv.runner.TBImgLoggerHook) for x in self._hooks)
        for i, data_batch in enumerate(data_loader):
            self._inner_iter = i
            self.call_hook('before_train_iter')
            outputs = self.batch_processor(
                self.model, data_batch, train_mode=True, log_img=log_img, **kwargs)
            ...
            if log_img and self._inner_iter%self._hooks[-1].interval==0:
                # import pdb; pdb.set_trace()
                self.img_log_buffer.append(outputs['tf_logs'])
                self.img_log_buffer.append(self.kwargs['img_norm_cfg'])
                self.img_log_buffer.append(data_batch['img_meta'].data[0][0])
                outputs.pop('tf_logs')
    ...
    def val(self, data_loader, **kwargs):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            with torch.no_grad():
                outputs = self.batch_processor(
                    self.model, data_batch, train_mode=False, log_img=False, **kwargs)
            if not isinstance(outputs, dict):
                raise TypeError('batch_processor() must return a dict')
            if 'log_vars' in outputs:
                self.log_buffer.update(outputs['log_vars'],
    ...
    def register_training_hooks(self,
        ...
        if cnp_mem_config is not None:
            self.register_hook(self.build_hook(cnp_mem_config, MeMCNPHook))
